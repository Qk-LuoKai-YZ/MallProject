 ...
 * FileName:NewBeeMallShoppingCartServiceImpl
 * Author:  7ink
 * Data:    2024/5/15 22:39
 * Description:购物车的业务层实现代码
 * History:
    ...
    public List<NewBeeMallShoppingCartItemVO> getMyShoppingCartItems(Long newBeeMallUserId) {
        ...
        if (!CollectionUtils.isEmpty(newBeeMallShoppingCartItems)) {
            ...
            if (!CollectionUtils.isEmpty(newBeeMallGoods)) {
                // 使用Function.identity()作为Map的value，即每个元素本身作为value。
                // 使用(entity1, entity2) -> entity1作为冲突解决策略，当出现重复的key时，保留第一个出现的元素。
